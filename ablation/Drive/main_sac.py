...
def parse_configs():
    ...
    parser.add_argument('--config', default="Driver/cfgs/sac_default.yml",
...
def train():
    ...
    env.set_model(pretrained=not cfg.no_pretrained, weight_file=cfg.ENV.env_model)
...
def test():
    ...
    # Use the same metrics as taa/metrics.py
    try:
        os.makedirs(output_dir, exist_ok=True)
        metric = AnticipationMetric(fpr_max=cfg.fpr_max, vis_list=[], output_dir=output_dir)
        # prepare inputs for AnticipationMetric.calculate
        preds_list = [all_pred_scores[i] for i in range(all_pred_scores.shape[0])]
        labels_list = [bool(all_gt_labels[i] > 0) for i in range(all_gt_labels.shape[0])]
        abnormal_start_inds = [0 for _ in range(all_pred_scores.shape[0])]
        accident_inds = [int(all_toas[i]) for i in range(all_toas.shape[0])]
        eval_results = metric.calculate(preds_list, labels_list, abnormal_start_inds, accident_inds, sep="#")
        # Print key metrics
        keys_to_show = [k for k in eval_results.keys() if any(s in k for s in ["AUC#", "mAUC#", "AP_full#", "mAP_full#", "tta#", "mtta#", "threshold#"])]
        for k in keys_to_show:
            print(f"[AnticipationMetric] {k}: {eval_results[k]:.4f}")
    except Exception as e:
        print(f"AnticipationMetric evaluation failed: {e}")
